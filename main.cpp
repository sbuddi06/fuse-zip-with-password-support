 ...
 */
void print_usage() {
    fprintf(stderr, "usage: %s [options] <zip-file> <mountpoint> <passwd> \n\n", PROGRAM);
    fprintf(stderr,
            "general options:\n"
    ...
    switch (key) {
        ...
        case FUSE_OPT_KEY_NONOPT: {
            ...
            switch (param->strArgCount) {
                ...
                default:
                    fprintf(stderr, "%s: only three arguments allowed: filename, mountpoint and password\n", PROGRAM);
                    return ERROR;
            }
        }
...
int main(int argc, char *argv[]) {
    ...
    if (!param.version) {
        ...
        // try password
        if (param.usePasswd) {
            if (data->try_passwd(param.passwd)){
